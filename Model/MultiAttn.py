...
'''
class MultiAttnLayer_QA(nn.Module):
...
#         query_modality = self.norm_1(query_modality)
#         modality_A = self.norm_2(modality_A)
        
        attn_output_1 = self.add_norm_1(query_modality, lambda query_modality: self.attn_1(query_modality, modality_A, modality_A))
        QA_output = self.add_norm_2(attn_output_1, self.ff)

        return  QA_output
    
    
class MultiAttnLayer_QB(nn.Module):
    ...
    def __init__(self, num_heads, model_dim, hidden_dim, dropout_rate):
        ...
        self.ff = Feedforward(model_dim, hidden_dim, dropout_rate)
        self.add_norm_4 = AddNorm(model_dim, dropout_rate)
    ...
    def forward(self, query_modality, modality_A, modality_B):
#         query_modality = self.norm_1(query_modality)
#         modality_B = self.norm_2(modality_B)
        attn_output_2 = self.add_norm_3(query_modality, lambda query_modality: self.attn_2(query_modality, modality_B, modality_B))
        QB_output = self.add_norm_4(attn_output_2, self.ff)
...
'''
class MultiAttn_QA(nn.Module):

    def __init__(self, num_layers, model_dim, num_heads, hidden_dim, dropout_rate):
        super().__init__()

        self.multiattn_layers_QA = nn.ModuleList([
            MultiAttnLayer_QA(num_heads, model_dim, hidden_dim, dropout_rate) for _ in range(num_layers)])

    def forward(self, query_modality, modality_A, modality_B):
        for multiattn_layer in self.multiattn_layers_QA:
            query_modality = multiattn_layer(query_modality, modality_A, modality_B)
        
        return query_modality

class MultiAttn_QB(nn.Module):

    def __init__(self, num_layers, model_dim, num_heads, hidden_dim, dropout_rate):
        super().__init__()

        self.multiattn_layers_QB = nn.ModuleList([
            MultiAttnLayer_QB(num_heads, model_dim, hidden_dim, dropout_rate) for _ in range(num_layers)])

    def forward(self, query_modality, modality_A, modality_B):
        for multiattn_layer in self.multiattn_layers_QB:
            query_modality = multiattn_layer(query_modality, modality_A, modality_B)
...
class MultiAttnModel(nn.Module):
    ...
    def __init__(self, num_layers, model_dim, num_heads, hidden_dim, dropout_rate):
        ...
        self.multiattn_text_audio = MultiAttn_QA(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
        self.multiattn_text_visual = MultiAttn_QB(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
        
        self.multiattn_audio_text = MultiAttn_QA(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
    
        self.multiattn_visual_text = MultiAttn_QA(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
    
    def forward(self, text_features, audio_features, visual_features):
        t_a = self.multiattn_text_audio(text_features, audio_features, visual_features)
        t_v = self.multiattn_text_visual(text_features, audio_features, visual_features)
        
        a_t = self.multiattn_audio_text(audio_features, text_features, visual_features)
    
        v_t = self.multiattn_visual_text(visual_features, text_features, audio_features)
        
        return t_a, t_v, a_t, v_t
...
#         self.multiattn_audio_text = MultiAttn_QA(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
#         self.multiattn_audio_visual = MultiAttn_QB(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
        
#         self.multiattn_visual_text = MultiAttn_QA(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
#         self.multiattn_visual_audio = MultiAttn_QB(num_layers, model_dim, num_heads, hidden_dim, dropout_rate)
...
#         a_t = self.multiattn_audio_text(audio_features, text_features, visual_features)
#         a_v = self.multiattn_audio_visual(audio_features, text_features, visual_features)
        
#         v_t = self.multiattn_visual_text(visual_features, text_features, audio_features)
#         v_a = self.multiattn_visual_audio(visual_features, text_features, audio_features)

#         return t_a, t_v, a_t,a_v,v_t,v_a
