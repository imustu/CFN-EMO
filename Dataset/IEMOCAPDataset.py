...
class IEMOCAPDataset(Dataset):
    ...
    def __init__(self, train = True):
        ...
        '''
        train-->True，self.keys--->self.trainVid
        train-->False，self.keys--->self.self.testVid
        '''
    ...
    def collate_fn(self, data):
        ...
        for i in dat:
            temp = dat[i].values
            if i <= 3:
                output.append(pad_sequence([temp[i] for i in range(len(temp))], padding_value = 0))   #speaker_mask
            elif i <= 4:
                output.append(pad_sequence([temp[i] for i in range(len(temp))], True, padding_value = 0))  #utt_mask
            elif i <= 5:
                output.append(pad_sequence([temp[i] for i in range(len(temp))], True, padding_value = -1))  #label_mask
